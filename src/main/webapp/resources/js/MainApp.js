...
app.config(function($routeProvider) {
	...
	.when('/searchmod1', {
		...
		templateUrl : '/dsign/signatories',
		controller : 'signatoriesController'
	}).when('/profile', {
		templateUrl : '/dsign/profile',
	}).otherwise({
		redirectTo : "/Home"
	})
})
...
app.controller('reportmod1Controller', [ '$scope', '$window', '$http',
		'$filter', function($scope, $window, $http, $filter) {
...
app.controller('reportmod2Controller', [ '$scope', '$window', '$http',
		'$filter', function($scope, $window, $http, $filter) {
...
app.controller('profileController', [ '$scope', '$http',
	function($scope, $http) {
		$http({
			...
			$scope.doclist = response.data.object;
		}, function errorcallback(response) {
			$scope.error = response.statusText;
		});
		...
		$http({
			...
			$scope.signlist = response.data.object;
		}, function errorcallback(response) {
			$scope.error = response.statusText;
		});
